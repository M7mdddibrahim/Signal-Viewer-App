...
class MyWindow(QtWidgets.QMainWindow):
    ...
    def Move1(self):
        oldplot = self.GetChosenPlotLine1()
        if oldplot == -1 or len(PlotLines1) == 0:
            self.ErrorMsg("No Signal Chosen")
            return

        if len(PlotLines2) == 0:
            self.ErrorMsg("No target plot available in Channel 2")
            return

        newplot = PlotLines2[-1]  # Get the target plot
        if newplot.data is None:
            self.ErrorMsg("Target plot is empty")
            return

        if oldplot.data is not None:
            # Copy data from old plot to new plot
            newplot.data = oldplot.data.copy()
            newplot.data_line.setData(
                newplot.data["time"] * self.zoomFactorChannel2,
                newplot.data["amplitude"] * self.zoomFactorChannel2
            )
    ...
    def Move2(self):
        ...
        oldplot = PlotLines1[-1]  # Get the target plot
        if oldplot.data is None:
            self.ErrorMsg("Target plot is empty")
            return

        if newplot.data is not None:
            # Copy data from new plot to old plot
            oldplot.data = newplot.data.copy()
            oldplot.data_line.setData(
                oldplot.data["time"] * self.zoomFactorChannel1,
                oldplot.data["amplitude"] * self.zoomFactorChannel1
            )
            oldplot.index = newplot.index
